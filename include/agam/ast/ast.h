...
struct SourceLocation {
    ...
    SourceLocation(const std::string &f, int l, int c) : filename(f), line(l), column(c) {}
};
...
struct TypeInfo {
    ...
    bool operator==(const TypeInfo &other) const {
        return kind == other.kind && isArray == other.isArray && elementType == other.elementType &&
               arraySize == other.arraySize && isStruct == other.isStruct &&
               structName == other.structName && isEnum == other.isEnum &&
               enumName == other.enumName && pointerDepth == other.pointerDepth &&
               isSlice == other.isSlice && isGeneric == other.isGeneric &&
               genericName == other.genericName && genericArgs == other.genericArgs &&
               pulseTag == other.pulseTag && isMutable == other.isMutable;
    }
    ...
    bool operator<(const TypeInfo &other) const {
        ...
        if (genericName != other.genericName)
            return genericName < other.genericName;
        if (genericArgs != other.genericArgs)
            return genericArgs < other.genericArgs;
        if (pulseTag != other.pulseTag)
            return pulseTag < other.pulseTag;
    ...
    static TypeInfo scalar(TypeKind k) {
        TypeInfo ti;
        ti.kind = k;
        return ti;
    }
    /// Create a fixed-size array type: [elementType; size]
    static TypeInfo array(TypeKind elem, int size) { return array(scalar(elem), size); }
    ...
    static TypeInfo slice(TypeKind elem) { return slice(scalar(elem)); }
...
inline std::string normalizeTypeName(const std::string &name) {
    ...
    while (start < suffix.length()) {
        std::string typeName =
            suffix.substr(start, (end == std::string::npos) ? std::string::npos : end - start);
...
inline std::string typeInfoToString(const TypeInfo &t) {
    if (t.kind == TypeKind::Unknown && !t.isArray && !t.isSlice && !t.isStruct && !t.isEnum &&
        !t.isGeneric) {
        return "Unknown";
    }
...
class MethodCallExpr : public Expr {
  public:
    ...
    MethodCallExpr(std::unique_ptr<Expr> base, std::string methodName,
        ...
        : base(std::move(base)), methodName(std::move(methodName)), args(std::move(args)),
          genericArgs(std::move(genArgs)) {}
    void accept(ASTVisitor &visitor) override;
    std::unique_ptr<ASTNode> clone() const override;
};
...
class IndexExpr : public Expr {
  public:
    ...
    IndexExpr(std::unique_ptr<Expr> base, std::unique_ptr<Expr> idx,
              std::unique_ptr<Expr> end = nullptr, bool range = false)
...
class IndexAssignExpr : public Expr {
  public:
    ...
    std::unique_ptr<Expr> value;
    IndexAssignExpr(std::unique_ptr<Expr> base, std::unique_ptr<Expr> idx,
...
class CastExpr : public Expr {
  public:
    ...
    TypeInfo targetType;
    CastExpr(std::unique_ptr<Expr> e, TypeInfo t) : expr(std::move(e)), targetType(std::move(t)) {}
    void accept(ASTVisitor &visitor) override;
    std::unique_ptr<ASTNode> clone() const override;
};
...
class VarDeclStmt : public Stmt {
  public:
    ...
    VarDeclStmt(std::string name, TypeInfo ti, std::unique_ptr<Expr> init = nullptr,
                bool isMut = false)
        : name(std::move(name)), typeInfo(std::move(ti)), initializer(std::move(init)),
          isMutable(isMut) {}
    void accept(ASTVisitor &visitor) override;
    std::unique_ptr<ASTNode> clone() const override;
};
...
class FunctionDecl : public ASTNode {
  public:
    ...
    FunctionDecl(std::string name, std::vector<std::string> typeParams, std::vector<Param> params,
                 TypeInfo retType, std::unique_ptr<BlockStmt> body, bool isExtern = false,
                 bool isMut = false)
        : name(std::move(name)), typeParams(std::move(typeParams)), params(std::move(params)),
          returnTypeInfo(std::move(retType)), body(std::move(body)), isExtern(isExtern),
          isMutating(isMut) {}
    void accept(ASTVisitor &visitor) override;
    std::unique_ptr<ASTNode> clone() const override;
};
...
class StructDecl : public ASTNode {
  public:
    ...
    StructDecl(std::string name, std::vector<std::string> typeParams,
               std::vector<StructField> fields)
...
class EnumDecl : public ASTNode {
  public:
    ...
    EnumDecl(std::string name, std::vector<std::string> typeParams,
             std::vector<EnumVariant> variants)
...
class ImplDecl : public ASTNode {
  public:
    ...
    TypeInfo targetType;
    std::vector<std::unique_ptr<FunctionDecl>> methods;
    ImplDecl(std::string traitName, std::vector<std::string> typeParams, TypeInfo targetType,
             std::vector<std::unique_ptr<FunctionDecl>> methods)
        : traitName(std::move(traitName)), typeParams(std::move(typeParams)),
          targetType(targetType), methods(std::move(methods)) {}
    void accept(ASTVisitor &visitor) override;
    std::unique_ptr<ASTNode> clone() const override;
};
