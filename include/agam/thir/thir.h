...
class ThirStringLiteral : public ThirExpr {
  public:
    std::string value;
    explicit ThirStringLiteral(std::string v) : value(std::move(v)) {
        typeInfo = TypeInfo::scalar(TypeKind::String);
    }
};
...
class ThirNullLiteral : public ThirExpr {
  public:
    ThirNullLiteral() {
        typeInfo = TypeInfo::scalar(
            TypeKind::Unknown); /* will be updated during type checking if needed */
    }
};
...
class ThirVarRef : public ThirExpr {
  public:
    HirId defId;
    std::string name;
    ThirVarRef(HirId id, std::string name, TypeInfo t) : defId(id), name(std::move(name)) {
        typeInfo = t;
    }
};
...
class ThirBinaryExpr : public ThirExpr {
  public:
    ...
    ThirBinaryExpr(BinaryOp op, std::unique_ptr<ThirExpr> l, std::unique_ptr<ThirExpr> r,
                   TypeInfo t)
...
class ThirCallExpr : public ThirExpr {
  public:
    ...
    ThirCallExpr(HirId id, std::string name, std::vector<std::unique_ptr<ThirExpr>> args,
                 TypeInfo retType)
...
class ThirCastExpr : public ThirExpr {
  public:
    ...
    ThirCastExpr(std::unique_ptr<ThirExpr> op, TypeInfo from, TypeInfo to)
        : operand(std::move(op)), fromTypeInfo(from) {
        typeInfo = to;
    }
};
...
class ThirIndexExpr : public ThirExpr {
  public:
    ...
    ThirIndexExpr(std::unique_ptr<ThirExpr> base, std::unique_ptr<ThirExpr> index, TypeInfo t,
                  std::unique_ptr<ThirExpr> end = nullptr, bool range = false)
...
class ThirIndexAssign : public ThirExpr {
  public:
    ...
    std::unique_ptr<ThirExpr> index;
    std::unique_ptr<ThirExpr> value;
    ThirIndexAssign(std::unique_ptr<ThirExpr> base, std::unique_ptr<ThirExpr> index,
                    std::unique_ptr<ThirExpr> value, TypeInfo t)
...
struct ThirSlicePtr : public ThirExpr {
    std::unique_ptr<ThirExpr> slice;
    ThirSlicePtr(std::unique_ptr<ThirExpr> s, TypeInfo t) : slice(std::move(s)) { typeInfo = t; }
};
...
class ThirFieldAccess : public ThirExpr {
  public:
    ...
    ThirFieldAccess(std::unique_ptr<ThirExpr> base, std::string field, int idx, TypeInfo t)
        : base(std::move(base)), field(std::move(field)), fieldIndex(idx) {
        typeInfo = t;
    }
};
...
class ThirNewExpr : public ThirExpr {
  public:
    ...
    ThirNewExpr(TypeInfo allocType, TypeInfo resultType, std::unique_ptr<ThirExpr> size = nullptr)
        : allocatedType(std::move(allocType)), sizeExpr(std::move(size)) {
        typeInfo = resultType;
    }
};
...
class ThirAllocExpr : public ThirExpr {
  public:
    ...
    std::string zoneName;
    ThirAllocExpr(TypeInfo allocType, std::unique_ptr<ThirExpr> c, std::string z,
                  TypeInfo resultType)
...
class ThirVarDecl : public ThirStmt {
  public:
    ...
    ThirVarDecl(HirId id, std::string name, TypeInfo typeInfo,
                std::unique_ptr<ThirExpr> init = nullptr)
...
class ThirEnumVariantExpr : public ThirExpr {
  public:
    ...
    ThirEnumVariantExpr(std::string ename, std::string vname, int vidx, std::unique_ptr<ThirExpr> p,
                        TypeInfo t)
        : enumName(std::move(ename)), variantName(std::move(vname)), variantIndex(vidx),
          payload(std::move(p)) {
        typeInfo = t;
    }
};
...
class ThirFor : public ThirStmt {
  public:
    ...
    std::unique_ptr<ThirBlock> body;
    ThirFor(HirId id, std::string name, TypeInfo vTy, std::unique_ptr<ThirExpr> iter,
            std::unique_ptr<ThirBlock> body)
        : varId(id), varName(std::move(name)), varType(vTy), iterable(std::move(iter)),
          body(std::move(body)) {}
};
...
class ThirFuncDecl {
  public:
    ...
    ThirFuncDecl(HirId id, std::string name, std::vector<ThirParam> params, TypeInfo retType,
                 std::unique_ptr<ThirBlock> body, bool isExtern = false)
