...
void TypeChecker::error(const SourceLocation &loc, const std::string &message) {
    if (diag_) diag_->error(loc, message);
}
...
TypeInfo TypeChecker::resolveType(const TypeInfo &type, const SourceLocation &loc) {
    ...
    if ((type.isStruct || type.kind == TypeKind::Unknown) && !type.isGeneric) {
        ...
        if (name.empty() && type.kind == TypeKind::Unknown) return type;
    ...
    if (!type.pulseTag.empty()) {
        if (!symbols_.isZoneInScope(type.pulseTag)) {
            error(loc, "அறியப்படாத அல்லது செயலற்ற மண்டலம் (Unknown region) '" + type.pulseTag + "' துடிப்பு அடையாளமாக (pulse tag) பயன்படுத்தப்பட்டுள்ளது");
        }
    }
...
void TypeChecker::visit(BorrowExpr &node) {
    ...
    if (node.isMutable) {
        if (!lastExprMutable_) {
            error(node.loc, "மாற்ற முடியாத (immutable) மூலத்திலிருந்து மாற்றக்கூடிய (mutable) கடனைப் பெற முடியாது");
        }
        if (!varName.empty()) {
            auto info = symbols_.lookup(varName);
            if (info && (info->sharedBorrowCount > 0 || info->mutablyBorrowed)) {
                error(node.loc, "'" + varName + "' ஏற்கனவே கடன் வாங்கப்பட்டிருக்கும் நிலையில் மாற்றக்கூடிய வகையில் (mutably) கடன் வாங்க முடியாது");
            }
        ...
        if (!varName.empty()) {
            ...
            if (info && info->mutablyBorrowed) {
                error(node.loc, "'" + varName + "' மாற்றக்கூடிய வகையில் (mutably) கடன் வாங்கப்பட்டிருக்கும் நிலையில் கடன் வாங்க முடியாது");
            }
    ...
    lastExprMutable_ = node.isMutable; // A pointer itself is an L-value, but here it's an R-value result
}
...
void TypeChecker::visit(EscapeExpr &node) {
    ...
    if (!varName.empty()) {
        auto info = symbols_.lookup(varName);
        if (info && (info->sharedBorrowCount > 0 || info->mutablyBorrowed)) {
            error(node.loc, "நினைவக பாதுகாப்பு மீறல் (Memory safety violation): '" + varName + "' (அல்லது அதன் புலங்கள்) கடன் வாங்கப்பட்டிருக்கும் நிலையில் தப்பிக்க (escape) முடியாது");
        }
    }

    if (!symbols_.isZoneInScope(node.destinationZone)) {
        error(node.loc, "இலக்கு மண்டலம் (Destination zone) '" + node.destinationZone + "' எல்லைக்குள் இல்லை");
    }
...
void TypeChecker::visit(Program &node) {
    ...
    for (size_t i = 0; i < node.functions.size(); ++i) {
        ...
        for (auto &p : fn->params) info.paramTypes.push_back(p.typeInfo);
    ...
    for (size_t i = 0; i < node.impls.size(); ++i) {
        ...
        for (auto &fn : id->methods) {
            ...
            mi.isFunction = true;
            for (auto &p : fn->params) mi.paramTypes.push_back(p.typeInfo);
                    ...
                    if (auto *call = dynamic_cast<CallExpr*>(es->expr.get())) {
                        ...
                        if (mc->methodName == "print" || mc->methodName == "println") {
                             ...
                             if (!symbols_.findImplForMethod(baseTi, mc->methodName)) {
                                 if (expandStructuralPrinting(mc->base.get(), mc->methodName, expanded)) {
                                     continue;
                                 }
                             }
                        }
...
void TypeChecker::visit(TraitDecl &node) {
    ...
    for (auto &method : node.methods) {
        ...
        mi.isFunction = true;
        for (auto &p : method->params) mi.paramTypes.push_back(p.typeInfo);
        info.methods.push_back(mi);
    }
...
void TypeChecker::visit(ConstDecl &node) {
    ...
    if (node.value) {
        ...
        if (auto reason = isCompatible(node.typeInfo, initType); !reason.empty()) {
            error(node.loc, "நிலைமாறிலி (Constant) '" + node.name + "' ஐத் தொடங்க முடியாது: " + reason);
        }
    } else {
        error(node.loc, "நிலைமாறிலி (Constant) '" + node.name + "' ஒரு தொடக்க மதிப்பைக் கொண்டிருக்க வேண்டும்");
    }
...
void TypeChecker::visit(FunctionDecl &node) {
    activeTypeParams_.clear();
    for (const auto &p : node.typeParams) activeTypeParams_.insert(p);
    ...
    currentReturnType_ = prevReturnType;
    for (const auto &p : node.typeParams) activeTypeParams_.erase(p);
    symbols_.exitScope();
}
...
void TypeChecker::visit(ReturnStmt &node) {
    if (node.value) {
        ...
        if (!retType.pulseTag.empty()) {
            error(node.loc, "நினைவக பாதுகாப்பு மீறல் (Memory safety violation): உள்ளூர் மண்டலத்திலிருந்து (local zone) '~" + retType.pulseTag + "' சுட்டியைத் திரும்பப் பெற முடியாது");
        }
...
void TypeChecker::visit(DeleteStmt &node) {
    ...
    if (lastExprInfo_.pointerDepth == 0 && lastExprInfo_.kind != TypeKind::Unknown) {
        error(node.loc, "delete requires a pointer operand, got '" + typeInfoToString(lastExprInfo_) + "'");
    }
}
...
void TypeChecker::visit(BinaryExpr &node) {
    ...
    if (auto reason = isCompatible(lhs.kind, rhs.kind); !reason.empty()) {
        error(node.loc, "இருமை செயலி வகைகள் பொருந்தவில்லை (Binary operator types mismatch): " + reason);
    }
    ...
    if (node.op == BinaryOp::Eq || node.op == BinaryOp::Neq ||
        ...
        node.op == BinaryOp::Lte || node.op == BinaryOp::Gte) {
        lastExprInfo_ = TypeInfo::scalar(TypeKind::Bool);
        ...
        bool isStringConcat = (node.op == BinaryOp::Add &&
                               lhs.kind == TypeKind::String && rhs.kind == TypeKind::String);
        if (!isStringConcat && (!isNumeric(lhs.kind) || !isNumeric(rhs.kind))) {
            error(node.loc, "கணித செயலிகளுக்கு எண் வகை தருமங்கள் (numeric operands) தேவை, ஆனால் கிடைத்துள்ளது '" +
...
void TypeChecker::visit(CastExpr &node) {
    ...
    bool valid = false;
    if (isNumeric(from.kind) && isNumeric(to.kind)) valid = true;
    else if (from.pointerDepth > 0 && to.pointerDepth > 0) valid = true;
    else if (from.pointerDepth > 0 && isNumeric(to.kind)) valid = true; // pointer to int
    else if (isNumeric(from.kind) && to.pointerDepth > 0) valid = true; // int to pointer
    else if (isCompatible(to, from).empty()) valid = true;
    
    if (!valid) {
        error(node.loc, "தவறான வகை மாற்றம் (invalid cast): '" + typeInfoToString(from) + "' இலிருந்து '" + typeInfoToString(to) + "' க்கு மாற்ற முடியாது");
    }
...
void TypeChecker::visit(CallExpr &node) {
    ...
    if (!info) {
        if (node.callee == "printf" || node.callee == "printf_int" || node.callee == "printf_float" || 
            node.callee == "printf_hex" || node.callee == "printf_bin" ||
            node.callee == "scanf_int" || node.callee == "scanf_int64" || node.callee == "scanf_float" ||
            node.callee == "agam_getchar" || node.callee == "agam_putchar" || node.callee == "agam_readline" ||
            node.callee == "fprintf_stderr" || node.callee == "fprintf_stderr_int" || node.callee == "fprintf_stderr_float" ||
    ...
    if (!info->typeParams.empty() && node.genericArgs.empty()) {
        for (const auto &tp : info->typeParams) {
            ...
            for (size_t i = 0; i < info->paramTypes.size(); i++) {
                if (i < node.args.size() && info->paramTypes[i].isGeneric && info->paramTypes[i].genericName == tp) {
...
void TypeChecker::visit(MethodCallExpr &node) {
    ...
    if (impl) {
        for (const auto &fn : impl->methods) {
            if (fn.name == node.methodName) {
                ...
                if (!baseVar.empty()) {
                    if (fn.isMutating) {
                        ...
                        if (isPointerReceiver) {
                            bool hasMutableAccess = varInfo->typeInfo.isMutable || varInfo->isMutable;
                            if (!hasMutableAccess) {
                                error(node.loc, "பகிர்வு கடனின் (shared borrow) வழியாக '" + baseVar +
                                      "' மாற்றக்கூடிய செயல்முறையினை (mutating method) '" + node.methodName +
                            ...
                            if (varInfo && !varInfo->isMutable) {
                                error(node.loc, "மாற்ற முடியாத மாறியின் (immutable variable) மீது '" + baseVar +
                                      "' மாற்றக்கூடிய செயல்முறையினை (mutating method) '" + node.methodName + "' அழைக்க முடியாது");
                            }
                        }
                ...
                if (undPos != std::string::npos) sdName = sdName.substr(0, undPos);
                ...
                if (sd && !sd->typeParams.empty()) {
                    ...
                    if (args.empty() && undPos != std::string::npos) {
                        ...
                        while (start < suffix.length()) {
                            ...
                            if (typeName == "int64") args.push_back(TypeInfo::scalar(TypeKind::Int64));
                            else if (typeName == "int32") args.push_back(TypeInfo::scalar(TypeKind::Int32));
                            else if (typeName == "float64") args.push_back(TypeInfo::scalar(TypeKind::Float64));
                            else if (typeName == "bool") args.push_back(TypeInfo::scalar(TypeKind::Bool));
                            else if (typeName == "string") args.push_back(TypeInfo::scalar(TypeKind::String));
                            else {
                ...
                std::string typeStr;
                if (baseType.isStruct) typeStr = baseType.structName;
                else if (baseType.isEnum) typeStr = baseType.enumName;
                else if (impl->targetType.isStruct) typeStr = impl->targetType.structName;
                else if (impl->targetType.isEnum) typeStr = impl->targetType.enumName;
                else typeStr = typeKindToString(baseType.kind);
                
                if (impl->traitName.empty()) {
                    node.resolvedMangledName = "_Inherent_" + typeStr + "_" + node.methodName;
                } else {
                    node.resolvedMangledName = "_Impl_" + impl->traitName + "_for_" + typeStr + "_" + node.methodName;
                }
                return;
            }
        }
    ...
    if ((baseType.isArray || baseType.isSlice || baseType.kind == TypeKind::String) && node.methodName == "len") {
        if (!node.args.empty()) {
            error(node.loc, "அணி/சிறுதுண்டின் (array/slice) 'நீளம்' (len) செயல்முறை 0 அளபுருக்களை எதிர்பார்க்கிறது");
        }
    ...
    error(node.loc, "வகை (type) '" + baseType.structName + "' இல் செயல்முறை (method) '" + node.methodName + "' காணப்படவில்லை");
    lastExprInfo_ = TypeInfo::scalar(TypeKind::Unknown);
}
...
void TypeChecker::visit(AssignExpr &node) {
    ...
    if (info) {
        if (!info->isMutable) {
            error(node.loc, "மாற்ற முடியாத மாறி (non-mutable variable) '" + node.name + "' க்கு மதிப்பு ஒதுக்க முடியாது");
        }
    }
    ...
    if (auto reason = isCompatible(info->typeInfo, valType); !reason.empty()) {
        error(node.loc, "'" + node.name + "' க்கான ஒதுக்கீட்டில் வகை பொருந்தவில்லை (type mismatch): " + reason);
    }

    // ZPM Safety Check
    if (!info->typeInfo.pulseTag.empty() && !valType.pulseTag.empty() && info->typeInfo.pulseTag != valType.pulseTag) {
        // If assigning a fresh allocation (empty/any tag) to a tagged field, it's allowed.
        if (valType.pulseTag.find("~") != std::string::npos || info->typeInfo.pulseTag == "Global") {
            // Allowed
        } else {
...
void TypeChecker::visit(ArrayLiteralExpr &node) {
    ...
    for (size_t i = 1; i < node.elements.size(); ++i) {
        ...
        if (auto reason = isCompatible(elemType, lastExprInfo_); !reason.empty()) {
            error(node.elements[i]->loc, "அணி மாறிலியில் (array literal) வகைகள் பொருந்தவில்லை: " + reason);
        }
    }

    lastExprInfo_ = TypeInfo();
    lastExprInfo_.kind = TypeKind::Unknown; // The kind of the array itself is technically Unknown/Not-a-Scalar
    lastExprInfo_.isArray = true;
...
void TypeChecker::visit(IndexExpr &node) {
    ...
    if (!baseTI.isArray && !baseTI.isSlice && baseTI.pointerDepth == 0) {
        error(node.loc, "அணி/சுட்டியல்லாத வகை (non-array/non-pointer type) '" + typeInfoToString(baseTI) + "' இல் அட்டவணை (index) பயன்படுத்த முடியாது");
    }

    node.index->accept(*this);
    if (!isNumericType(lastExprInfo_.kind)) {
        error(node.loc, "அணி அட்டவணை (array index) எண்ணாக இருக்க வேண்டும், ஆனால் '" + typeInfoToString(lastExprInfo_) + "' உள்ளது");
    }
    ...
    if (node.isRange) {
        if (node.endIndex) {
            ...
            if (!isNumericType(lastExprInfo_.kind)) {
                error(node.loc, "அணி சிறுதுண்டின் இறுதி அட்டவணை (slice end index) எண்ணாக இருக்க வேண்டும், ஆனால் '" + typeInfoToString(lastExprInfo_) + "' உள்ளது");
            }
        }
        // Slicing returns a slice of the same element type
        lastExprInfo_ = TypeInfo::slice(baseTI.elementType);
        lastExprMutable_ = false; // slices are typically immutable views in many designs, or depends on base.
                                 // For now, let's say they are like base.
...
void TypeChecker::visit(IndexAssignExpr &node) {
    ...
    if (!baseMutable) {
        ...
        if (baseTI.pointerDepth == 0) {
             error(node.base->loc, "அணி மாற்ற முடியாதது (immutable) என்பதால் அதன் உறுப்பிற்கு மதிப்பு ஒதுக்க முடியாது");
        }
    }
    ...
    node.index->accept(*this);
    if (!isNumericType(lastExprInfo_.kind)) {
        error(node.loc, "array index must be numeric, got '" + typeInfoToString(lastExprInfo_) + "'");
    }
    ...
    if (!compatError.empty()) {
        error(node.loc, "அணி ஒதுக்கீட்டில் வகை பொருந்தவில்லை (Type mismatch in array assignment): '" + 
              ...
              typeInfoToString(targetElementType) + "' வகை உறுப்பிற்கு ஒதுக்க முடியாது. " + compatError);
    }
}
...
void TypeChecker::visit(FieldAssignExpr &node) {
    ...
    TypeInfo baseTI = lastExprInfo_;
    if (!lastExprMutable_) {
        error(node.loc, "மாற்ற முடியாத பொருளின் (non-mutable object) புலத்திற்கு (field) மதிப்பு ஒதுக்க முடியாது");
    }
    ...
    if (!found) {
        error(node.loc, "அமைப்பு (Struct) '" + baseTI.structName + "' இல் '" + node.field + "' என்ற புலம் (field) இல்லை");
        return;
    }
    ...
    if (auto reason = isCompatible(fieldType, valType); !reason.empty()) {
         error(node.loc, "'" + node.field + "' புலத்திற்கான ஒதுக்கீட்டில் வகை பொருந்தவில்லை (Type mismatch): " + reason);
    }

    // ZPM Safety Check
    if (!fieldType.pulseTag.empty() && !valType.pulseTag.empty() && fieldType.pulseTag != valType.pulseTag) {
        if (valType.pulseTag.find("~") != std::string::npos || fieldType.pulseTag == "Global" || fieldType.pulseTag.empty()) {
            // Allowed
        } else {
...
void TypeChecker::visit(EnumVariantExpr &node) {
    ...
    if (!variant) {
        error(node.loc, "பட்டியல் (Enum) '" + node.enumName + "' இல் '" + node.variantName + "' என்ற மாற்றின் (variant) காணப்படவில்லை");
        lastExprInfo_ = TypeInfo::scalar(TypeKind::Unknown);
        return;
    }
    ...
    if (node.payload) {
        if (!variant->hasPayload) {
            error(node.loc, "மாற்றின் (variant) '" + node.variantName + "' சுமை (payload) எதனையும் ஏற்காது");
        } else {
            node.payload->accept(*this);
            TypeInfo எதிர்பார்க்கிறோம்Payload = variant->payloadType;
            if (!node.genericArgs.empty()) {
                எதிர்பார்க்கிறோம்Payload = substitute(எதிர்பார்க்கிறோம்Payload, en->typeParams, node.genericArgs);
            }
            if (auto reason = isCompatible(எதிர்பார்க்கிறோம்Payload, lastExprInfo_); !reason.empty()) {
                error(node.loc, "மாற்றின் (variant) '" + node.variantName + "' க்கான சுமை வகை (payload type) பொருந்தவில்லை: " + reason);
            }
        }
    } else if (variant->hasPayload) {
        error(node.loc, "மாற்றின் (variant) '" + node.variantName + "' ஒரு சுமையினை (payload) எதிர்பார்க்கிறது");
    }
...
void TypeChecker::visit(MatchExpr &node) {
    ...
    if (!valType.isEnum) {
        error(node.loc, "'பொருத்து' (match) கோவை ஒரு பட்டியலாக (enum) இருக்க வேண்டும், ஆனால் '" + typeInfoToString(valType) + "' உள்ளது");
        lastExprInfo_ = TypeInfo::scalar(TypeKind::Unknown);
        return;
    }
    ...
    for (auto &arm : node.arms) {
        ...
        if (!variant) {
            error(node.loc, "பட்டியல் (Enum) '" + valType.enumName + "' இல் '" + arm.variantName + "' என்ற மாற்றின் (variant) காணப்படவில்லை");
            continue;
        }
        ...
        if (arm.hasBinding) {
            if (!variant->hasPayload) {
                error(arm.body->loc, "மாற்றின் (variant) '" + arm.variantName + "' சுமை (payload) எதனையும் கொண்டிருக்கவில்லை");
            } else {
                SymbolInfo binding;
                binding.name = arm.bindingName;
                binding.typeInfo = variant->payloadType;
                if (!valType.genericArgs.empty()) {
                    binding.typeInfo = substitute(binding.typeInfo, en->typeParams, valType.genericArgs);
                }
...
void TypeChecker::visit(NewExpr &node) {
    ...
    if (node.sizeExpr) {
        ...
        if (lastExprInfo_.kind != TypeKind::Int && lastExprInfo_.kind != TypeKind::Float) { // allow float for now? 
...
std::string TypeChecker::isCompatible(TypeKind எதிர்பார்க்கிறோம், TypeKind actual) const {
    if (எதிர்பார்க்கிறோம் == actual) return "";
    if (isNumeric(எதிர்பார்க்கிறோம்) && isNumeric(actual)) return "";
    if (எதிர்பார்க்கிறோம் == TypeKind::Unknown || actual == TypeKind::Unknown) return "";
    if (எதிர்பார்க்கிறோம் == TypeKind::Generic || actual == TypeKind::Generic) return "";
    return "எதிர்பார்க்கிறோம் (Expected) '" + std::string(typeKindToString(எதிர்பார்க்கிறோம்)) + "', ஆனால் கிடைத்தது (got) '" + std::string(typeKindToString(actual)) + "'";
}
...
std::string TypeChecker::isCompatible(const TypeInfo &எதிர்பார்க்கிறோம், const TypeInfo &actual) const {
    ...
    if (எதிர்பார்க்கிறோம்.isGeneric || actual.isGeneric) return "";
    
    if (எதிர்பார்க்கிறோம்.isArray != actual.isArray) return "எதிர்பார்க்கிறோம் array, got non-array type";
    if (எதிர்பார்க்கிறோம்.isSlice != actual.isSlice) return "எதிர்பார்க்கிறோம் slice, got non-slice type";
    if (எதிர்பார்க்கிறோம்.pointerDepth != actual.pointerDepth) {
        return "pointer depth mismatch: எதிர்பார்க்கிறோம் " + std::to_string(எதிர்பார்க்கிறோம்.pointerDepth) + 
               ", got " + std::to_string(actual.pointerDepth);
    }
    
    if (actual.kind == TypeKind::Null && எதிர்பார்க்கிறோம்.pointerDepth > 0) return "";
    if (எதிர்பார்க்கிறோம்.kind == TypeKind::Null && actual.pointerDepth > 0) return "";

    if (எதிர்பார்க்கிறோம்.isStruct != actual.isStruct) {
        return "எதிர்பார்க்கிறோம் struct, got non-struct type";
    }
    if (எதிர்பார்க்கிறோம்.isEnum != actual.isEnum) return "எதிர்பார்க்கிறோம் enum, got non-enum type";
    ...
    if (எதிர்பார்க்கிறோம்.isStruct) {
        ...
        if (e_pos != std::string::npos) e_base = e_base.substr(0, e_pos);
        if (a_pos != std::string::npos) a_base = a_base.substr(0, a_pos);
        
        if (e_base != a_base) {
            return "struct mismatch: '" + normalizeTypeName(எதிர்பார்க்கிறோம்.structName) + 
                   "' vs '" + normalizeTypeName(actual.structName) + "'";
        }
            
        if (எதிர்பார்க்கிறோம்.genericArgs.size() != actual.genericArgs.size()) {
            if (எதிர்பார்க்கிறோம்.genericArgs.empty() || actual.genericArgs.empty()) return ""; // Template vs Instance
    ...
    if (எதிர்பார்க்கிறோம்.isEnum) {
        if (எதிர்பார்க்கிறோம்.enumName != actual.enumName) {
             return "பட்டியல் வகை பொருந்தவில்லை (enum mismatch): '" + எதிர்பார்க்கிறோம்.enumName + "' vs '" + actual.enumName + "'";
        }
        return "";
    }
    ...
    if (எதிர்பார்க்கிறோம்.kind != actual.kind) {
        if (isNumeric(எதிர்பார்க்கிறோம்.kind) && isNumeric(actual.kind)) return "";
        return "வகை பொருந்தவில்லை (type mismatch): எதிர்பார்க்கிறோம் (Expected) '" + typeInfoToString(எதிர்பார்க்கிறோம்) + 
               "', ஆனால் கிடைத்தது (got) '" + typeInfoToString(actual) + "'";
    }
...
bool TypeChecker::expandPrintCall(CallExpr *call, std::vector<std::unique_ptr<Stmt>> &expanded) {
    if (call->callee != "print" && call->callee != "println" && 
        call->callee != "பதிப்பி" && call->callee != "வரியிறக்கி_பதிப்பி") return false;
    ...
    if (call->args.size() > 1) {
        for (size_t i = 0; i < call->args.size(); ++i) {
            ...
            if (i == call->args.size() - 1) {
                ...
                target = (call->callee == "பதிப்பி" || call->callee == "வரியிறக்கி_பதிப்பி") ? "பதிப்பி" : "print";
            }
    ...
    if (symbols_.findImplForMethod(ti, call->callee)) {
        auto mc = std::make_unique<MethodCallExpr>(std::move(call->args[0]), call->callee, std::vector<std::unique_ptr<Expr>>());
        mc->loc = call->loc;
...
bool TypeChecker::expandStructuralPrinting(Expr* receiver, const std::string& methodName, std::vector<std::unique_ptr<Stmt>> &expanded) {
        ...
        if (sd) {
            ...
            for (size_t j = 0; j < sd->fields.size(); ++j) {
                ...
                if (j < sd->fields.size() - 1) {
                    auto pc = std::make_unique<CallExpr>("print", std::vector<std::unique_ptr<Expr>>());
...
TypeInfo TypeChecker::substitute(const TypeInfo &type, const std::vector<std::string> &params, const std::vector<TypeInfo> &args) {
    TypeInfo result = type;
    if (result.isGeneric) {
    ...
    // If the struct itself is generic and its name matches a parameter (shouldn't happen for structs, but just in case)
    if (result.isStruct) {
        for (size_t i = 0; i < params.size(); ++i) {
...
int TypeChecker::getZoneDepth(const std::string &tag) const {
    if (tag.empty()) return -1; // Global/Eternal
    const auto &stack = symbols_.zoneStack();
    for (int i = 0; i < (int)stack.size(); ++i) {
        if (stack[i] == tag) return i;
    }
    return 9999; // Inactive or unknown zone
}

bool TypeChecker::isZoneCompatible(const std::string &srcTag, const std::string &tgtTag, const SourceLocation &loc) {
    if (srcTag == tgtTag) return true;
...
bool TypeChecker::canRead(const std::string &name, const SourceLocation &loc) {
    ...
    if (info->mutablyBorrowed) {
        error(loc, "மாறி '" + name + "' மாற்றக்கூடிய வகையில் (mutably) கடன் வாங்கப்பட்டிருக்கும் போது அதனைப் படிக்க முடியாது");
        return false;
    }
    return true;
}
