...
static std::string typeStr(const TypeInfo &ti) {
    std::string s = "";
    for (int i = 0; i < ti.pointerDepth; ++i) s += "*";
    if (ti.isArray) {
        s += std::string("[") + typeKindToString(ti.elementType) + "; " + std::to_string(ti.arraySize) + "]";
    } else if (ti.isStruct) {
        s += ti.structName;
    } else if (ti.isEnum) {
        s += ti.enumName;
...
std::string MirPrinter::printFunction(const MirFunction &func) {
    ...
    for (const auto &local : func.locals) {
        if (local.id < func.params.size()) continue; // simple heuristic if params are first
        out << "    let ";
        if (local.isTemp) out << "mut ";
        out << "%" << local.id;
        if (!local.name.empty()) out << " /* " << local.name << " */";
    ...
    for (auto &bb : func.blocks) {
                ...
                if constexpr (std::is_same_v<StatementT, MirAssign>) {
                    ...
                    std::visit([&](auto &&rv) {
                        ...
                        if constexpr (std::is_same_v<RValueT, MirRvalueUse>) {
                            ...
                            out << operandStr(rv.lhs) << " " << binaryOpToString(rv.op) << " " << operandStr(rv.rhs);
                        } else if constexpr (std::is_same_v<RValueT, MirRvalueUnaryOp>) {
                            out << (rv.op == UnaryOp::Negate ? "-" : (rv.op == UnaryOp::Not ? "!" : (rv.op == UnaryOp::AddressOf ? "&" : "*"))) << operandStr(rv.operand);
                        } else if constexpr (std::is_same_v<RValueT, MirRvalueCast>) {
                            out << operandStr(rv.operand) << " as " << typeStr(rv.toTypeInfo);
                            ...
                            for (size_t i = 0; i < rv.fields.size(); i++) {
                                if (i > 0) out << ", ";
                                out << rv.fields[i].name << ": " << operandStr(rv.fields[i].value);
                            }
                            out << " }";
                        } else if constexpr (std::is_same_v<RValueT, MirRvalueFieldAccess>) {
                        ...
                        } else if constexpr (std::is_same_v<RValueT, MirRvalueEnumPayload>) {
                            ...
                            out << operandStr(rv.base) << "[" << operandStr(rv.start) << ".." << operandStr(rv.end) << "]";
                            ...
                            out << "alloc<" << typeKindToString(rv.allocatedType.kind) << ">~" << rv.zoneName << "(" << operandStr(rv.count) << ")";
                        } else if constexpr (std::is_same_v<RValueT, MirRvalueBorrow>) {
                            out << (rv.isMutable ? "borrow mut " : "borrow ") << operandStr(rv.target);
                        } else if constexpr (std::is_same_v<RValueT, MirRvalueEscape>) {
                            out << "escape " << operandStr(rv.target) << " to " << rv.destinationZone;
                        } else if constexpr (std::is_same_v<RValueT, MirRvaluePtrOffset>) {
                            out << "ptr_offset(" << operandStr(rv.base) << ", " << rv.byteOffset << ") as " << typeStr(rv.resultType);
                        }
                    }, s.rvalue);
                    out << "\n";
                } else if constexpr (std::is_same_v<StatementT, MirIndexAssign>) {
                    out << "        " << operandStr(s.base) << "[" << operandStr(s.index) << "] = " << operandStr(s.value) << "\n";
                } else if constexpr (std::is_same_v<StatementT, MirFieldAssign>) {
                    out << "        " << operandStr(s.base) << "." << s.field << " = " << operandStr(s.value) << "\n";
        ...
        std::visit([&](auto &&term) {
            ...
            if constexpr (std::is_same_v<T, MirGoto>) {
                ...
                out << "switchInt(" << operandStr(term.discriminant)
                    << ") -> [true: bb" << term.thenBlock
                    << ", false: bb" << term.elseBlock << "]";
