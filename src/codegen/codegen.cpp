...
bool CodeGenerator::generate(MirProgram &program) {
    ...
    for (auto &cn : program.constants) {
        ...
        if (init) {
            new llvm::GlobalVariable(*module_, ty, true, llvm::GlobalValue::ExternalLinkage, init,
                                     cn.name);
        }
    }
...
llvm::Type *CodeGenerator::toLLVMType(const TypeInfo &typeInfo) {
    ...
    default:
        ...
        if (typeInfo.kind == TypeKind::Int)
            return llvm::Type::getInt32Ty(*context_);
        if (typeInfo.kind == TypeKind::Float)
            return llvm::Type::getDoubleTy(*context_);
        return llvm::Type::getInt32Ty(*context_);
    }
}
...
void CodeGenerator::emitFunctionSignature(MirFunction &func) {
    ...
    llvm::FunctionType *funcType =
        llvm::FunctionType::get(toLLVMType(func.returnTypeInfo), paramTypes, false);
...
void CodeGenerator::emitFunctionBody(MirFunction &func) {
    if (func.isExtern)
        return; // Extern functions only have a declaration, no body.
...
void CodeGenerator::emitStatement(const MirStatement &stmt, llvm::Function *func) {
    if (auto *assign = std::get_if<MirAssign>(&stmt)) {
            ...
            if (destType->isArrayTy() && rval->getType()->isArrayTy() &&
                ...
                for (unsigned i = 0; i < numElems; ++i) {
                    ...
                    if (elem->getType() != destElemType) {
                        ...
                        } else if (elem->getType()->isFloatingPointTy() &&
                                   destElemType->isFloatingPointTy()) {
                ...
                if (destType->isIntegerTy() && rval->getType()->isIntegerTy() &&
                    destType != rval->getType()) {
                    rval = builder_->CreateIntCast(rval, destType, true, "assign_cast");
                } else if (destType->isFloatingPointTy() && rval->getType()->isFloatingPointTy() &&
                           destType != rval->getType()) {
        ...
        if (idxAssign->base.typeInfo.isSlice) {
            ...
            llvm::Value *gep = builder_->CreateGEP(toLLVMType(idxAssign->elementTypeInfo), ptr,
                                                   index, "slicegep_assign");
        ...
        if (idxAssign->base.kind != MirOperand::Kind::Copy)
            return;
        ...
        llvm::Value *gep =
            builder_->CreateGEP(baseAlloca->getAllocatedType(), baseAlloca, indices, "idxassign");
        builder_->CreateStore(rval, gep);
    } else if (auto *fieldAssign = std::get_if<MirFieldAssign>(&stmt)) {
        if (fieldAssign->base.kind != MirOperand::Kind::Copy)
            return;
        ...
        if (fieldAssign->base.typeInfo.pointerDepth > 0 && fieldAssign->base.typeInfo.isStruct) {
            llvm::Value *ptr =
                builder_->CreateLoad(baseAlloca->getAllocatedType(), baseAlloca, "deref_ptr");
            auto sit = structTypes_.find(fieldAssign->base.typeInfo.structName);
            if (sit == structTypes_.end())
                return;
            llvm::Value *fieldPtr =
                builder_->CreateStructGEP(sit->second, ptr, fieldAssign->fieldIndex, "fieldptr");
            builder_->CreateStore(rval, fieldPtr);
        } else {
            llvm::Value *zero = llvm::ConstantInt::get(llvm::Type::getInt32Ty(*context_), 0);
            llvm::Value *idxValue =
                llvm::ConstantInt::get(llvm::Type::getInt32Ty(*context_), fieldAssign->fieldIndex);
            std::vector<llvm::Value *> indices = {zero, idxValue};
            llvm::Value *gep = builder_->CreateGEP(baseAlloca->getAllocatedType(), baseAlloca,
                                                   indices, "fieldassign");
            builder_->CreateStore(rval, gep);
        }
        ...
        llvm::FunctionCallee freeFunc = module_->getOrInsertFunction(
            "free", llvm::FunctionType::get(llvm::Type::getVoidTy(*context_),
        ...
        llvm::FunctionCallee createFunc = module_->getOrInsertFunction(
            "agam_zone_create",
        ...
        if (it != activeZones_.end()) {
            ...
            llvm::FunctionCallee destroyFunc = module_->getOrInsertFunction(
                "agam_zone_destroy",
        ...
        [this, &op](auto &&c) -> llvm::Value * {
            ...
            if constexpr (std::is_same_v<T, MirConstInt>) {
                ...
                return llvm::ConstantPointerNull::get(
                    llvm::cast<llvm::PointerType>(toLLVMType(op.typeInfo)));
            }
            return nullptr;
        },
        op.constant);
    ...
    return std::visit(
        [this, func](auto &&r) -> llvm::Value * {
            ...
            if constexpr (std::is_same_v<T, MirRvalueUse>) {
                ...
                case BinaryOp::Mod:
                    return isFloat ? builder_->CreateFRem(lhs, rhs, "remtmp")
                                   : builder_->CreateSRem(lhs, rhs, "remtmp");
                case BinaryOp::Eq:
                    return isFloat ? builder_->CreateFCmpOEQ(lhs, rhs, "eqtmp")
                                   : builder_->CreateICmpEQ(lhs, rhs, "eqtmp");
                case BinaryOp::Neq:
                    return isFloat ? builder_->CreateFCmpONE(lhs, rhs, "neqtmp")
                                   : builder_->CreateICmpNE(lhs, rhs, "neqtmp");
                case BinaryOp::Lt:
                ...
                if (r.op == UnaryOp::AddressOf) {
                    if (r.operand.kind == MirOperand::Kind::Copy) {
                        ...
                        if (it != localAllocas_.end()) {
                            ...
                            if (r.operand.typeInfo.isArray &&
                                r.operand.typeInfo.elementType != TypeKind::Unknown) {
                                llvm::Type *sliceTy =
                                    toLLVMType(TypeInfo::slice(r.operand.typeInfo.elementType));
                                ...
                                llvm::Value *firstElemPtr = builder_->CreateGEP(
                                    toLLVMType(r.operand.typeInfo), ptr, indices, "slice_ptr");

                                llvm::Value *lenVal =
                                    llvm::ConstantInt::get(llvm::Type::getInt64Ty(*context_),
                                                           r.operand.typeInfo.arraySize);
                                slice = builder_->CreateInsertValue(slice, firstElemPtr, 0,
                                                                    "slice_ptr_init");
                                slice =
                                    builder_->CreateInsertValue(slice, lenVal, 1, "slice_len_init");
                                return slice;
                            }
                            return ptr;
                        }
                ...
                if (r.base.kind != MirOperand::Kind::Copy)
                    return nullptr;
                ...
                if (r.base.typeInfo.isSlice) {
                    ...
                    llvm::Value *gep =
                        builder_->CreateGEP(toLLVMType(r.elementTypeInfo), ptr, index, "slicegep");
                ...
                if (r.base.typeInfo.pointerDepth > 0) {
                    ...
                    llvm::Value *index = emitOperand(r.index);
                    llvm::Value *gep =
                        builder_->CreateGEP(toLLVMType(r.elementTypeInfo), ptr, index, "ptrgep");
                ...
                for (auto &f : r.fields) {
                    ...
                    if (!elemVal)
                        return nullptr;
                    structVal =
                        builder_->CreateInsertValue(structVal, elemVal, f.fieldIndex, "structinit");
                }
                return structVal;
                ...
                if (r.base.typeInfo.pointerDepth > 0 && r.base.typeInfo.isStruct) {
                    ...
                    if (sit == structTypes_.end())
                        return nullptr;
                    llvm::Value *fieldPtr =
                        builder_->CreateStructGEP(sit->second, ptr, r.fieldIndex, "fieldptr");
                ...
                llvm::Value *tagVal =
                    llvm::ConstantInt::get(llvm::Type::getInt32Ty(*context_), r.variantIndex);
                llvm::Value *tagPtr =
                    builder_->CreateStructGEP(enumTy, tmpAlloca, 0, "enum_tag_ptr");
                ...
                if (r.hasPayload) {
                    ...
                    llvm::Value *payloadFieldPtr =
                        builder_->CreateStructGEP(enumTy, tmpAlloca, 1, "enum_payload_ptr");
                ...
                llvm::Value *payloadFieldPtr =
                    builder_->CreateStructGEP(enumTy, tmpAlloca, 1, "ext_payload_ptr");
                ...
                if (r.isDynamicArray) {
                    ...
                    if (numElems->getType() != llvm::Type::getInt64Ty(*context_)) {
                        numElems = builder_->CreateIntCast(
                            numElems, llvm::Type::getInt64Ty(*context_), false, "size_cast");
                    }
                    uint64_t elemSize = module_->getDataLayout().getTypeAllocSize(elemType);
                    llvm::Value *elemSizeVal =
                        llvm::ConstantInt::get(llvm::Type::getInt64Ty(*context_), elemSize);
                ...
                llvm::FunctionCallee mallocFunc = module_->getOrInsertFunction(
                    "malloc", llvm::FunctionType::get(llvm::PointerType::getUnqual(*context_),
                ...
                if (r.isDynamicArray) {
                    ...
                    if (lenVal->getType() != llvm::Type::getInt64Ty(*context_)) {
                        lenVal = builder_->CreateIntCast(lenVal, llvm::Type::getInt64Ty(*context_),
                                                         false, "len_cast");
                    }
                ...
                llvm::Value *subPtr =
                    builder_->CreateGEP(toLLVMType(TypeInfo::scalar(r.base.typeInfo.elementType)),
                                        ptr, {start}, "sub_ptr");
                ...
                llvm::FunctionCallee strlenFunc = module_->getOrInsertFunction(
                    "strlen",
                ...
                if (count->getType() != llvm::Type::getInt64Ty(*context_)) {
                    count = builder_->CreateIntCast(count, llvm::Type::getInt64Ty(*context_), false,
                                                    "count_cast");
                }
                uint64_t elemSize = module_->getDataLayout().getTypeAllocSize(elemType);
                llvm::Value *elemSizeVal =
                ...
                llvm::FunctionCallee allocFunc = module_->getOrInsertFunction(
                    "agam_zone_alloc",
                ...
                llvm::Value *offset =
                    llvm::ConstantInt::get(llvm::Type::getInt64Ty(*context_), r.byteOffset);

                // In LLVM with opaque pointers, we just use GEP with i8 as element type
                llvm::Value *offsetted =
                    builder_->CreateGEP(llvm::Type::getInt8Ty(*context_), base, offset);
            ...
            if constexpr (std::is_same_v<T, MirGoto>) {
                ...
                if (cond) {
                    ...
                    if (cond->getType()->isIntegerTy(32)) {
                        cond = builder_->CreateICmpNE(
                            cond,
                            llvm::ConstantInt::get(builder_->getContext(), llvm::APInt(32, 0)),
                            "tobool");
                    }
                ...
                if (cond) {
                    llvm::SwitchInst *switchInst =
                        builder_->CreateSwitch(cond, blockMap[t.defaultBlock], t.targets.size());
                    for (auto &pair : t.targets) {
                        llvm::ConstantInt *onVal =
                            llvm::ConstantInt::get(llvm::Type::getInt32Ty(*context_), pair.first);
