...
void FloatLiteralExpr::accept(ASTVisitor &v) { v.visit(*this); }
std::unique_ptr<ASTNode> FloatLiteralExpr::clone() const {
    auto copy = std::make_unique<FloatLiteralExpr>(value);
    copy->loc = loc;
    return copy;
}
void StringLiteralExpr::accept(ASTVisitor &v) { v.visit(*this); }
...
void NullLiteralExpr::accept(ASTVisitor &v) { v.visit(*this); }
...
std::unique_ptr<ASTNode> BinaryExpr::clone() const {
    auto copy = std::make_unique<BinaryExpr>(op, 
        std::unique_ptr<Expr>(static_cast<Expr*>(lhs->clone().release())),
...
std::unique_ptr<ASTNode> UnaryExpr::clone() const {
    auto copy = std::make_unique<UnaryExpr>(op,
...
std::unique_ptr<ASTNode> AssignExpr::clone() const {
    auto copy = std::make_unique<AssignExpr>(name, 
...
std::unique_ptr<ASTNode> IndexExpr::clone() const {
    auto copy = std::make_unique<IndexExpr>(
        ...
        endIndex ? std::unique_ptr<Expr>(static_cast<Expr*>(endIndex->clone().release())) : nullptr,
        isRange);
    copy->loc = loc;
    return copy;
}
void IndexAssignExpr::accept(ASTVisitor &v) { v.visit(*this); }
...
void StructLiteralExpr::accept(ASTVisitor &v) { v.visit(*this); }
std::unique_ptr<ASTNode> StructLiteralExpr::clone() const {
    std::vector<StructFieldInit> clonedFields;
    for (const auto &f : fields) {
        clonedFields.push_back({f.name, std::unique_ptr<Expr>(static_cast<Expr*>(f.value->clone().release()))});
    }
...
void FieldAssignExpr::accept(ASTVisitor &v) { v.visit(*this); }
...
void EnumVariantExpr::accept(ASTVisitor &v) { v.visit(*this); }
std::unique_ptr<ASTNode> EnumVariantExpr::clone() const {
    auto copy = std::make_unique<EnumVariantExpr>(enumName, variantName, genericArgs,
...
void DerefAssignExpr::accept(ASTVisitor &v) { v.visit(*this); }
...
std::unique_ptr<ASTNode> NewExpr::clone() const {
    auto copy = std::make_unique<NewExpr>(
        allocatedType,
        sizeExpr ? std::unique_ptr<Expr>(static_cast<Expr*>(sizeExpr->clone().release())) : nullptr);
    copy->loc = loc;
    return copy;
}
...
std::unique_ptr<ASTNode> MethodCallExpr::clone() const {
    ...
    auto copy = std::make_unique<MethodCallExpr>(
        std::unique_ptr<Expr>(static_cast<Expr*>(base->clone().release())),
        methodName, std::move(clonedArgs), genericArgs);
    copy->loc = loc;
    copy->resolvedMangledName = resolvedMangledName;
    return copy;
}
void ZoneExpr::accept(ASTVisitor &v) { v.visit(*this); }
std::unique_ptr<ASTNode> ZoneExpr::clone() const {
    auto copy = std::make_unique<ZoneExpr>(zoneName, 
        std::unique_ptr<BlockStmt>(static_cast<BlockStmt*>(body->clone().release())));
    copy->loc = loc;
    return copy;
}
void AllocExpr::accept(ASTVisitor &v) { v.visit(*this); }
std::unique_ptr<ASTNode> AllocExpr::clone() const {
    auto copy = std::make_unique<AllocExpr>(type,
...
void EscapeExpr::accept(ASTVisitor &v) { v.visit(*this); }
...
std::unique_ptr<ASTNode> ExprStmt::clone() const {
    auto copy = std::make_unique<ExprStmt>(std::unique_ptr<Expr>(static_cast<Expr*>(expr->clone().release())));
    copy->loc = loc;
    return copy;
}
void VarDeclStmt::accept(ASTVisitor &v) { v.visit(*this); }
std::unique_ptr<ASTNode> VarDeclStmt::clone() const {
    auto copy = std::make_unique<VarDeclStmt>(name, typeInfo,
        initializer ? std::unique_ptr<Expr>(static_cast<Expr*>(initializer->clone().release())) : nullptr, isMutable);
    copy->loc = loc;
    return copy;
}
...
std::unique_ptr<ASTNode> IfStmt::clone() const {
    auto copy = std::make_unique<IfStmt>(
        ...
        elseBranch ? std::unique_ptr<Stmt>(static_cast<Stmt*>(elseBranch->clone().release())) : nullptr);
    copy->loc = loc;
    return copy;
}
...
std::unique_ptr<ASTNode> ForStmt::clone() const {
    auto copy = std::make_unique<ForStmt>(varName,
...
std::unique_ptr<ASTNode> FunctionDecl::clone() const {
    auto copy = std::make_unique<FunctionDecl>(name, typeParams, params, returnTypeInfo,
        body ? std::unique_ptr<BlockStmt>(static_cast<BlockStmt*>(body->clone().release())) : nullptr,
        isExtern);
    copy->loc = loc;
    return copy;
}

void ConstDecl::accept(ASTVisitor &v) { v.visit(*this); }
std::unique_ptr<ASTNode> ConstDecl::clone() const {
    auto copy = std::make_unique<ConstDecl>(name, typeInfo,
        value ? std::unique_ptr<Expr>(static_cast<Expr*>(value->clone().release())) : nullptr);
    copy->loc = loc;
    return copy;
}
...
std::unique_ptr<ASTNode> TraitDecl::clone() const {
    ...
    for (const auto &m : methods) {
        clonedMethods.push_back(std::unique_ptr<FunctionDecl>(static_cast<FunctionDecl*>(m->clone().release())));
    }
...
std::unique_ptr<ASTNode> ImplDecl::clone() const {
    ...
    auto copy = std::make_unique<ImplDecl>(traitName, typeParams, targetType, std::move(clonedMethods));
    copy->loc = loc;
    return copy;
}
...
std::unique_ptr<ASTNode> Program::clone() const {
    ...
    for (const auto &f : functions) copy->functions.push_back(std::unique_ptr<FunctionDecl>(static_cast<FunctionDecl*>(f->clone().release())));
    for (const auto &c : constants) copy->constants.push_back(std::unique_ptr<ConstDecl>(static_cast<ConstDecl*>(c->clone().release())));
    for (const auto &s : structs) copy->structs.push_back(std::unique_ptr<StructDecl>(static_cast<StructDecl*>(s->clone().release())));
    for (const auto &e : enums) copy->enums.push_back(std::unique_ptr<EnumDecl>(static_cast<EnumDecl*>(e->clone().release())));
    for (const auto &t : traits) copy->traits.push_back(std::unique_ptr<TraitDecl>(static_cast<TraitDecl*>(t->clone().release())));
    for (const auto &i : impls) copy->impls.push_back(std::unique_ptr<ImplDecl>(static_cast<ImplDecl*>(i->clone().release())));
    return copy;
}
