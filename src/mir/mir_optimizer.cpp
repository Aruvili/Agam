...
int64_t MirOptimizer::getTypeSize(const TypeInfo &ti) {
    if (ti.pointerDepth > 0) return 8;
    if (ti.isSlice) return 16;
    if (ti.isArray) return getTypeSize(TypeInfo::scalar(ti.elementType)) * ti.arraySize;
    
    switch (ti.kind) {
        case TypeKind::Int8:   case TypeKind::UInt8:  return 1;
        case TypeKind::Int16:  case TypeKind::UInt16: return 2;
        case TypeKind::Int32:  case TypeKind::UInt32: return 4;
        case TypeKind::Int64:  case TypeKind::UInt64: return 8;
...
void MirOptimizer::fuseZoneAllocations(MirFunction &func) {
    for (auto &block : func.blocks) {
        ...
        for (auto &pair : blockAllocations) {
            ...
            masterAssign.rvalue = MirRvalueZoneAlloc{TypeInfo::scalar(TypeKind::Int8), MirOperand::makeConstInt64(0), zone};
            ...
            for (size_t idx : indices) {
                ...
                if (totalSize == 0) {
                    offsetAssign.rvalue = MirRvalueCast{MirOperand::makeCopy(masterLocal, masterType), masterType, func.locals[assign->destination.local].typeInfo};
                } else {
                    offsetAssign.rvalue = MirRvaluePtrOffset{MirOperand::makeCopy(masterLocal, masterType), totalSize, func.locals[assign->destination.local].typeInfo};
                }
            ...
            std::get<MirRvalueZoneAlloc>(masterAssign.rvalue).count = MirOperand::makeConstInt64(totalSize);
            insertions[indices[0]].push_back(masterAssign);
        }

        for (size_t i = 0; i < block.statements.size(); ++i) {
            if (insertions.count(i)) {
                for (auto &s : insertions[i]) finalStmts.push_back(s);
            }
