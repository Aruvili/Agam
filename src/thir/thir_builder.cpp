...
std::unique_ptr<ThirProgram> ThirBuilder::build(HirProgram &program, DiagnosticEngine &diag) {
    ...
    for (auto &ed : thirProg->enums) {
        for (auto &v : ed.variants) {
            if (v.hasPayload) v.payloadType = resolveTypeInfo(v.payloadType);
        }
    }
...
std::unique_ptr<ThirFuncDecl> ThirBuilder::lowerFunc(HirFuncDecl &node) {
    ...
    return std::make_unique<ThirFuncDecl>(
        node.id, node.name, std::move(thirParams), node.returnTypeInfo, std::move(body), node.isExtern);
}
...
std::unique_ptr<ThirBlock> ThirBuilder::lowerBlock(HirBlock &node) {
    ...
    for (auto &s : node.stmts) {
        ...
        if (thir) stmts.push_back(std::move(thir));
    }
...
std::unique_ptr<ThirStmt> ThirBuilder::lowerStmt(HirStmt &node) {
    ...
    if (auto *vd = dynamic_cast<HirVarDecl *>(&node)) {
        ...
        if (vd->initializer) {
            ...
            if (init && init->typeInfo.kind != TypeKind::Unknown && init->typeInfo != vd->typeInfo) {
                // Allow numeric conversions (int↔int, int→float, float32→float64)
                if (!vd->typeInfo.isArray && isNumericType(vd->typeInfo.kind) && isNumericType(init->typeInfo.kind)) {
                    TypeInfo originalType = init->typeInfo;
                    init = std::make_unique<ThirCastExpr>(
                        std::move(init), originalType, vd->typeInfo);
                }
                // Array element type mismatch: cast each element
                         ...
                         isNumericType(vd->typeInfo.elementType) && isNumericType(init->typeInfo.elementType)) {
                    ...
                    if (arrLit) {
                        ...
                        for (auto &elem : arrLit->elements) {
                            elem = std::make_unique<ThirCastExpr>(std::move(elem), fromElem, toElem);
                        }
                        arrLit->typeInfo = vd->typeInfo;
                    }
                }
                else if (init->typeInfo != vd->typeInfo) {
                    error(node.loc, "மாறி (Variable) '" + vd->name + "' ஐத் தொடங்க முடியாது: வகை பொருந்தவில்லை (type mismatch): எதிர்பார்க்கிறோம் (Expected) '" +
                          typeInfoToString(vd->typeInfo) + "', ஆனால் கிடைத்தது (got) '" +
    ...
    if (auto *ret = dynamic_cast<HirReturn *>(&node)) {
        ...
        if (ret->value) {
            ...
            if (val->typeInfo != currentReturnTypeInfo_ && val->typeInfo.kind != TypeKind::Unknown) {
                if (isNumericType(currentReturnTypeInfo_.kind) && isNumericType(val->typeInfo.kind)) {
                    TypeInfo originalType = val->typeInfo;
                    val = std::make_unique<ThirCastExpr>(
                        std::move(val), originalType, currentReturnTypeInfo_);
                } else {
                    error(node.loc, "திரும்பும் வகை பொருந்தவில்லை (Return type mismatch): எதிர்பார்க்கிறோம் (Expected) '" +
                          std::string(typeKindToString(currentReturnTypeInfo_.kind)) +
                          "', ஆனால் கிடைத்தது (got) '" + typeKindToString(val->typeInfo.kind) + "'");
                }
            }
        }
    ...
    if (auto *hf = dynamic_cast<HirFor *>(&node)) {
        ...
        auto stmt = std::make_unique<ThirFor>(hf->varId, hf->varName, elemType, std::move(iter), std::move(body));
        stmt->loc = node.loc;
        return stmt;
    }
    ...
    if (auto *delS = dynamic_cast<HirDeleteStmt *>(&node)) {
        ...
        if (pointer && pointer->typeInfo.pointerDepth == 0) {
            error(node.loc, "'நீக்கு' (delete) செயல்முறைக்கு ஒரு சுட்டி (pointer) தேவை, ஆனால் கிடைத்தது '" + typeInfoToString(pointer->typeInfo) + "'");
        }
...
std::unique_ptr<ThirExpr> ThirBuilder::lowerExpr(HirExpr &node) {
    ...
    if (auto *lit = dynamic_cast<HirNullLiteral *>(&node)) {
    ...
    if (auto *bin = dynamic_cast<HirBinaryExpr *>(&node)) {
        ...
        if (isComparison(bin->op)) {
            ...
            if (isNumericType(lt.kind) && isNumericType(rt.kind)) {
                // Promote if mixed
                if (lt != rt) {
                    TypeKind common = isFloatType(lt.kind) || isFloatType(rt.kind) ? TypeKind::Float64 :
                                     (typeSizeInBits(lt.kind) >= typeSizeInBits(rt.kind) ? lt.kind : rt.kind);
                    if (lt.kind != common) lhs = std::make_unique<ThirCastExpr>(std::move(lhs), lt, TypeInfo::scalar(common));
                    if (rt.kind != common) rhs = std::make_unique<ThirCastExpr>(std::move(rhs), rt, TypeInfo::scalar(common));
                }
                resultType = TypeInfo::scalar(TypeKind::Bool);
            } else if (lt == rt) {
                resultType = TypeInfo::scalar(TypeKind::Bool);
            } else {
                error(node.loc, std::string("வகை பொருந்தவில்லை (type mismatch): '") + typeKindToString(lt.kind) + 
                      "' மற்றும் '" + typeKindToString(rt.kind) + "' ஆகியவற்றை ஒப்பிட முடியாது");
            }
            ...
            if (lt.kind == TypeKind::Bool && rt.kind == TypeKind::Bool) {
                ...
                error(node.loc, "தருக்க செயலிகளுக்கு (logical operators) மெய்மை (bool) வகை இயக்கிகள் தேவை");
            }
        } else {
            // Arithmetic: +, -, *, /
            if (isNumericType(lt.kind) && isNumericType(rt.kind)) {
                if (isFloatType(lt.kind) || isFloatType(rt.kind)) {
                    ...
                    resultType = TypeInfo::scalar((lt.kind == TypeKind::Float64 || rt.kind == TypeKind::Float64) ? TypeKind::Float64 : TypeKind::Float32);
                    if (lt != resultType) lhs = std::make_unique<ThirCastExpr>(std::move(lhs), lt, resultType);
                    if (rt != resultType) rhs = std::make_unique<ThirCastExpr>(std::move(rhs), rt, resultType);
                } else {
                    // Both integers — use the wider type
                    resultType = TypeInfo::scalar(typeSizeInBits(lt.kind) >= typeSizeInBits(rt.kind) ? lt.kind : rt.kind);
                    if (lt != resultType) lhs = std::make_unique<ThirCastExpr>(std::move(lhs), lt, resultType);
                    if (rt != resultType) rhs = std::make_unique<ThirCastExpr>(std::move(rhs), rt, resultType);
                }
            } else if (bin->op == BinaryOp::Add && lt.kind == TypeKind::String && rt.kind == TypeKind::String) {
                ...
                error(node.loc, std::string("இருமை செயலி (binary operator) '") +
                      ...
                      typeKindToString(lt.kind) + "' மற்றும் '" + typeKindToString(rt.kind) + "'");
            }
        }

        auto e = std::make_unique<ThirBinaryExpr>(bin->op, std::move(lhs), std::move(rhs), resultType);
        e->loc = node.loc;
        return e;
    }
    ...
    if (auto *un = dynamic_cast<HirUnaryExpr *>(&node)) {
        ...
        if (un->op == UnaryOp::AddressOf) {
            ...
            if (t.pointerDepth == 0) {
                error(node.loc, "சுட்டி (pointer) அல்லாத வகை '" + typeInfoToString(t) + "' -ஐ குறிநீக்கம் (dereference) செய்ய முடியாது");
            } else {
                t.pointerDepth--;
            }
        } else if (un->op == UnaryOp::Negate && !isNumericType(t.kind)) {
            error(node.loc, "எண் அல்லாத வகை '" + std::string(typeKindToString(t.kind)) + "' -ஐ எதிர்மறையாக்க (negate) முடியாது");
    ...
    if (auto *call = dynamic_cast<HirCallExpr *>(&node)) {
        ...
        if (it != funcMap_.end()) {
            ...
            if (call->args.size() != it->second.paramTypes.size()) {
                error(node.loc, std::string("செயல்முறை (function) '") + call->calleeName + "' எதிர்பார்க்கும் அளபுருக்கள் " +
                      std::to_string(it->second.paramTypes.size()) + ", ஆனால் கிடைத்துள்ளது " +
        ...
        for (size_t i = 0; i < call->args.size(); i++) {
            ...
            if (it != funcMap_.end() && i < it->second.paramTypes.size()) {
                ...
                if (expected.pointerDepth == arg->typeInfo.pointerDepth + 1 ) {
                    ...
                    if (expected.kind == addrType.kind && expected.structName == addrType.structName) {
                        auto loc = arg->loc;
                        arg = std::make_unique<ThirUnaryExpr>(UnaryOp::AddressOf, std::move(arg), expected);
                        arg->loc = loc;
                    }
                }
                ...
                if (arg->typeInfo != expected && arg->typeInfo.kind != TypeKind::Unknown) {
                    if (isNumericType(expected.kind) && isNumericType(arg->typeInfo.kind)) {
                        ...
                        arg = std::make_unique<ThirCastExpr>(std::move(arg), originalType, expected);
                    } else {
                        error(node.loc, std::string("செயல்முறை '") +
                              call->calleeName + "' -ன் அளபுரு " + std::to_string(i+1) + ": '" +
                              typeKindToString(expected.kind) + "' எதிர்பார்க்கிறோம், ஆனால் கிடைத்துள்ளது '" +
        ...
        auto e = std::make_unique<ThirCallExpr>(call->calleeId, call->calleeName, std::move(thirArgs), retTypeInfo);
        e->loc = node.loc;
        return e;
    }
        ...
        if (it != typeMap_.end()) {
            ...
            if (val->typeInfo != targetType && val->typeInfo.kind != TypeKind::Unknown) {
                if (isNumericType(targetType.kind) && isNumericType(val->typeInfo.kind)) {
                    ...
                    error(node.loc, std::string("மாறியின் வகை '") + typeKindToString(targetType.kind) + "' -க்கு '" + 
                          typeKindToString(val->typeInfo.kind) + "' -ஐ ஒதுக்கீடு செய்ய முடியாது");
                }
            }
        }
        auto e = std::make_unique<ThirAssignExpr>(assign->targetId, assign->targetName, std::move(val), targetType);
        e->loc = node.loc;
        return e;
    }
        ...
        if (!arr->elements.empty()) {
            ...
            for (size_t i = 1; i < arr->elements.size(); i++) {
                ...
                if (elem->typeInfo.kind != elemKind) {
                    error(node.loc, std::string("அணி உறுப்புகள் ஒரே வகையாக இருக்க வேண்டும். எதிர்பார்க்கிறோம் '") +
    ...
    if (auto *idx = dynamic_cast<HirIndexExpr *>(&node)) {
        ...
        auto e = std::make_unique<ThirIndexExpr>(std::move(base), std::move(index), resType, std::move(endIndex), idx->isRange);
        e->loc = node.loc;
        return e;
    }
    ...
    if (auto *idxAssign = dynamic_cast<HirIndexAssign *>(&node)) {
        ...
        if (!base->typeInfo.isArray && !base->typeInfo.isSlice && base->typeInfo.pointerDepth == 0) {
        ...
        auto e = std::make_unique<ThirIndexAssign>(std::move(base), std::move(index), std::move(val), elemType);
        e->loc = node.loc;
        return e;
    }
    ...
    if (auto *slit = dynamic_cast<HirStructLiteral *>(&node)) {
        ...
        if (it == structDefs_.end()) {
            error(node.loc, std::string("அறியப்படாத அமைப்பு (struct) வகை '") + slit->structName + "'");
            return nullptr;
        }
        ...
        for (auto &f : slit->fields) {
            ...
            if (idx < 0) {
                error(node.loc, std::string("அமைப்பு '") + slit->structName + "' -ல் '" + f.name + "' என்ற புலம் (field) இல்லை");
                idx = 0;
            }
            ...
            if (!declTI.isStruct && !declTI.isArray && val->typeInfo.kind != declTI.kind && val->typeInfo.kind != TypeKind::Unknown) {
                if (isNumericType(declTI.kind) && isNumericType(val->typeInfo.kind)) {
                    ...
                    error(node.loc, std::string("புலம் (field) '") + f.name + "' '" + typeInfoToString(declTI) + "' -ஐ எதிர்பார்க்கிறது, ஆனால் கிடைத்துள்ளது '" + typeInfoToString(val->typeInfo) + "'");
                }
            }
    ...
    if (auto *fa = dynamic_cast<HirFieldAccess *>(&node)) {
        ...
        if ((base->typeInfo.isArray || base->typeInfo.isSlice) && fa->field == "len") {
             ...
             auto e = std::make_unique<ThirFieldAccess>(std::move(base), "len", idx, TypeInfo::scalar(TypeKind::Int));
             e->loc = node.loc;
             return e;
        }

        if (!base->typeInfo.isStruct) {
            error(node.loc, "அமைப்பு (struct) அல்லாத வகை '" + typeInfoToString(base->typeInfo) + "' -ல் புலத்தை அணுக முடியாது");
            return nullptr;
        }
        ...
        if (idx < 0) {
            error(node.loc, std::string("அமைப்பு '") + base->typeInfo.structName + "' -ல் '" + fa->field + "' என்ற புலம் இல்லை");
            idx = 0;
        }
    ...
    if (auto *fassign = dynamic_cast<HirFieldAssign *>(&node)) {
        ...
        if (!base) return nullptr;
        if (!base->typeInfo.isStruct) {
            error(node.loc, "அமைப்பு (struct) அல்லாத வகை '" + typeInfoToString(base->typeInfo) + "' -ல் புலத்திற்கு மதிப்பை ஒதுக்க முடியாது");
            return nullptr;
        }
        auto it = structDefs_.find(base->typeInfo.structName);
        if (it == structDefs_.end()) {
        ...
        if (idx < 0) {
            error(node.loc, std::string("அமைப்பு '") + base->typeInfo.structName + "' -ல் '" + fassign->field + "' என்ற புலம் இல்லை");
            idx = 0;
        }
        TypeInfo fieldType = sdef->fields[idx].typeInfo;
        auto val = lowerExpr(*fassign->value);
        if (!fieldType.isStruct && !fieldType.isArray && val->typeInfo.kind != fieldType.kind && val->typeInfo.kind != TypeKind::Unknown) {
            if (isNumericType(fieldType.kind) && isNumericType(val->typeInfo.kind)) {
                ...
                error(node.loc, std::string("புலத்தின் வகை '") + typeInfoToString(fieldType) + "' -க்கு '" + typeInfoToString(val->typeInfo) + "' -ஐ ஒதுக்கீடு செய்ய முடியாது (" + fassign->field + ")");
            }
        }
        auto e = std::make_unique<ThirFieldAssign>(std::move(base), fassign->field, idx, std::move(val));
        e->loc = node.loc;
        return e;
    }
    ...
    if (auto *da = dynamic_cast<HirDerefAssign *>(&node)) {
        ...
        if (val->typeInfo != expectedValType && val->typeInfo.kind != TypeKind::Unknown && expectedValType.kind != TypeKind::Unknown) {
            // Allow numeric coercion (e.g. int64 literal assigned to *int32 target)
            if (expectedValType != val->typeInfo && isNumericType(expectedValType.kind) && isNumericType(val->typeInfo.kind)) {
                TypeInfo originalType = val->typeInfo;
                val = std::make_unique<ThirCastExpr>(std::move(val), originalType, expectedValType);
            } else {
                error(node.loc, std::string("குறிநீக்க ஒதுக்கீட்டில் வகை பொருந்தவில்லை. எதிர்பார்க்கிறோம் '") + typeInfoToString(expectedValType) + "', ஆனால் கிடைத்தது '" + typeInfoToString(val->typeInfo) + "'");
            }
        }
    ...
    if (auto *eve = dynamic_cast<HirEnumVariantExpr *>(&node)) {
        ...
        if (vIdx < 0) {
            error(node.loc, std::string("பட்டியல் '") + eve->enumName + "' -ல் '" + eve->variantName + "' என்ற உருப்படி (variant) இல்லை");
            return nullptr;
        }
        ...
        if (vdef.hasPayload) {
            if (!eve->payload) {
                error(node.loc, std::string("உருப்படி '") + eve->variantName + "' '" + typeInfoToString(vdef.payloadType) + "' வகை சுமையை (payload) எதிர்பார்க்கிறது");
            } else {
                payloadNode = lowerExpr(*eve->payload);
                if (payloadNode && payloadNode->typeInfo.kind != vdef.payloadType.kind && payloadNode->typeInfo.kind != TypeKind::Unknown) {
                    // Allow numeric coercion (e.g. int64 literal for int32 payload)
                    if (isNumericType(payloadNode->typeInfo.kind) && isNumericType(vdef.payloadType.kind)) {
                        TypeInfo originalType = payloadNode->typeInfo;
                        payloadNode = std::make_unique<ThirCastExpr>(std::move(payloadNode), originalType, vdef.payloadType);
                    } else {
                        error(node.loc, std::string("உருப்படி '") + eve->variantName + "' '" + typeInfoToString(vdef.payloadType) + "' வகை சுமையை எதிர்பார்க்கிறது, ஆனால் கிடைத்தது '" + typeInfoToString(payloadNode->typeInfo) + "'");
                    }
                }
            }
        } else {
            if (eve->payload) {
                error(node.loc, std::string("உருப்படி '") + eve->variantName + "' சுமையை ஏற்றுக்கொள்வதில்லை");
            }
        }
        
        TypeInfo enumType = TypeInfo::namedEnum(eve->enumName);
        auto e = std::make_unique<ThirEnumVariantExpr>(eve->enumName, eve->variantName, vIdx, std::move(payloadNode), enumType);
        e->loc = node.loc;
        return e;
    }
    ...
    if (auto *me = dynamic_cast<HirMatchExpr *>(&node)) {
        ...
        if (!valNode) return nullptr;
        if (!valNode->typeInfo.isEnum) {
            error(me->value->loc, "பொருத்து (match) கோவையின் மதிப்பு பட்டியலாக (enum) இருக்க வேண்டும், ஆனால் கிடைத்தது '" + typeInfoToString(valNode->typeInfo) + "'");
            return nullptr;
        }
        std::string enumName = valNode ? valNode->typeInfo.enumName : "";
        auto it = enumDefs_.find(enumName);
        if (it == enumDefs_.end()) {
            if (valNode) error(node.loc, "அறியப்படாத பட்டியல் வகை '" + enumName + "'");
            return nullptr;
        }
        const HirEnumDef *edef = it->second;
        ...
        for (auto &arm : me->arms) {
            ...
            if (vIdx < 0) {
                error(node.loc, std::string("பட்டியல் '") + enumName + "' -ல் '" + arm.variantName + "' என்ற உருப்படி இல்லை");
                continue;
            }
            ...
            if (arm.hasBinding && !vdef.hasPayload) {
                error(node.loc, std::string("உருப்படி '") + arm.variantName + "' -ல் இணைப்பதற்கு சுமை இல்லை");
            }
            ...
            if (arm.exprBody) {
                ...
                if (tarm.exprBody) armType = tarm.exprBody->typeInfo;
            ...
            if (firstArm && armType.kind != TypeKind::Unknown && armType.kind != TypeKind::Void) {
                ...
                firstArm = false;
            } else if (!firstArm && matchTypeInfo != armType && armType.kind != TypeKind::Unknown && armType.kind != TypeKind::Void) {
                error(node.loc, std::string("பொருத்து கிளைகளின் வகைகள் பொருந்தவில்லை: எதிர்பார்க்கிறோம் '") + typeInfoToString(matchTypeInfo) + "', ஆனால் கிடைத்தது '" + typeInfoToString(armType) + "'");
            }
            
            thirArms.push_back(std::move(tarm));
        }
        
        auto e = std::make_unique<ThirMatchExpr>(std::move(valNode), std::move(thirArms), matchTypeInfo);
        e->loc = node.loc;
        return e;
    }
    ...
    if (auto *zoneE = dynamic_cast<HirZoneExpr *>(&node)) {
        ...
        auto e = std::make_unique<ThirZoneExpr>(zoneE->zoneName, std::move(body), TypeInfo::scalar(TypeKind::Void));
        e->loc = node.loc;
        return e;
    }
    ...
    if (auto *allocE = dynamic_cast<HirAllocExpr *>(&node)) {
        ...
        auto e = std::make_unique<ThirAllocExpr>(allocType, std::move(count), allocE->zoneName, resultType);
        e->loc = node.loc;
        return e;
    }
    ...
    if (auto *borrowE = dynamic_cast<HirBorrowExpr *>(&node)) {
        ...
        auto e = std::make_unique<ThirBorrowExpr>(borrowE->isMutable, std::move(target), resultType);
        e->loc = node.loc;
        return e;
    }
    ...
    if (auto *escapeE = dynamic_cast<HirEscapeExpr *>(&node)) {
        ...
        auto e = std::make_unique<ThirEscapeExpr>(std::move(target), escapeE->destinationZone, resultType);
        e->loc = node.loc;
        return e;
    }
    ...
    if (auto *castE = dynamic_cast<HirCastExpr *>(&node)) {
        ...
        bool valid = false;
        if (isNumericType(from.kind) && isNumericType(to.kind)) valid = true;
        else if (from.pointerDepth > 0 && to.pointerDepth > 0) valid = true;
        else if (from.pointerDepth > 0 && isNumericType(to.kind)) valid = true; // pointer to int
        else if (isNumericType(from.kind) && to.pointerDepth > 0) valid = true; // int to pointer (e.g., 0 as *int)
        else if (from == to) valid = true;
        
        if (!valid) {
            error(node.loc, "தவறான வகை நிலைமாற்றம் (cast): '" + typeInfoToString(from) + "' என்பதிலிருந்து '" + typeInfoToString(to) + "' -க்கு மாற்ற முடியாது");
        }
