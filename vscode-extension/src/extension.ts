...
export function activate(context: vscode.ExtensionContext) {
    ...
    const runFileCommand = vscode.commands.registerCommand('agam.runFile', () => {
        ...
        document.save().then(() => {
            ...
            const interpreterPath = config.get<string>('interpreterPath', 'agam');

            // Create or show terminal
            let terminal = vscode.window.terminals.find((t: vscode.Terminal) => t.name === 'agam');
            if (!terminal) {
    ...
    const openReplCommand = vscode.commands.registerCommand('agam.openRepl', () => {
        ...
        let terminal = vscode.window.terminals.find((t: vscode.Terminal) => t.name === 'agam REPL');
        if (!terminal) {
