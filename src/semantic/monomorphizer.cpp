...
void Monomorphizer::visit(Program &node) {
    for (size_t i = 0; i < node.functions.size(); ++i) {
        ...
        if (fn->typeParams.empty()) fn->accept(*this);
    }
...
void Monomorphizer::visit(StructLiteralExpr &node) {
    if (!node.structType.genericArgs.empty()) {
        ...
        node.structType.structName = getMangledName(node.structType.structName, node.structType.genericArgs);
...
void Monomorphizer::visit(ReturnStmt &node) { if (node.value) node.value->accept(*this); }
void Monomorphizer::visit(NewExpr &node) {
    if (!node.allocatedType.genericArgs.empty()) {
        if (node.allocatedType.isStruct) specializeStruct(node.allocatedType.structName, node.allocatedType.genericArgs);
        node.allocatedType.structName = getMangledName(node.allocatedType.structName, node.allocatedType.genericArgs);
...
std::string Monomorphizer::getMangledName(const std::string &base, const std::vector<TypeInfo> &args) {
    std::string name = base + "_";
    for (const auto &arg : args) {
        if (arg.isStruct) name += arg.structName;
        else if (arg.isEnum) name += arg.enumName;
        else name += typeKindToString(arg.kind);
        for (int i=0; i<arg.pointerDepth; ++i) name += "Ptr";
    }
    return name;
}
...
void Monomorphizer::mangleType(TypeInfo &ti) {
    if (!ti.genericArgs.empty()) {
        for (auto &arg : ti.genericArgs) mangleType(arg);
        
        if (ti.isStruct || ti.isEnum) {
            if (ti.isStruct) specializeStruct(ti.structName, ti.genericArgs);
            else if (ti.isEnum) specializeEnum(ti.enumName, ti.genericArgs);
            
            ti.structName = getMangledName(ti.isStruct ? ti.structName : ti.enumName, ti.genericArgs);
            if (ti.isEnum) ti.enumName = ti.structName;
            ti.genericArgs.clear();
        }
    }
}

TypeInfo Monomorphizer::substitute(const TypeInfo &type, const std::vector<std::string> &params, const std::vector<TypeInfo> &args) {
    ...
    if (type.isArray || type.isSlice) {
        for (size_t i = 0; i < params.size(); ++i) {
            bool matches = false;
            if (type.isGeneric && params[i] == type.genericName) matches = true;
            else if (type.isStruct && params[i] == type.structName) matches = true;
...
public:
    TypeSubstitutionVisitor(Monomorphizer &monomorphizer, const std::vector<std::string> &params, const std::vector<TypeInfo> &args)
    ...
    TypeInfo substitute(const TypeInfo &type) {
        ...
        if (type.isArray || type.isSlice) {
            for (size_t i = 0; i < params_.size(); ++i) {
                bool matches = false;
                if (type.isGeneric && params_[i] == type.genericName) matches = true;
                else if (type.isStruct && params_[i] == type.structName) matches = true;

                if (matches && i < args_.size()) {
                    TypeInfo res;
                    if (type.isSlice) res = TypeInfo::slice(args_[i]);
                    else res = TypeInfo::array(args_[i], type.arraySize);
    ...
    void visit(NewExpr &node) override {
        ...
        if (node.sizeExpr) node.sizeExpr->accept(*this);
    }
    ...
    void visit(BinaryExpr &node) override { node.lhs->accept(*this); node.rhs->accept(*this); }
    ...
    void visit(IndexExpr &node) override { node.base->accept(*this); node.index->accept(*this); }
    void visit(IndexAssignExpr &node) override { node.base->accept(*this); node.index->accept(*this); node.value->accept(*this); }
    void visit(FieldAccessExpr &node) override { node.base->accept(*this); }
    void visit(FieldAssignExpr &node) override { node.base->accept(*this); node.value->accept(*this); }
    void visit(MatchExpr &node) override { node.value->accept(*this); /* ... */ }
    void visit(DerefAssignExpr &node) override { node.pointer->accept(*this); node.value->accept(*this); }
    void visit(ExprStmt &node) override { node.expr->accept(*this); }
    void visit(BlockStmt &node) override { for (auto &s : node.statements) s->accept(*this); }
    void visit(ReturnStmt &node) override { if (node.value) node.value->accept(*this); }
    void visit(IfStmt &node) override { node.condition->accept(*this); node.thenBranch->accept(*this); if (node.elseBranch) node.elseBranch->accept(*this); }
    void visit(WhileStmt &node) override { node.condition->accept(*this); node.body->accept(*this); }
    ...
    void visit(FunctionDecl &node) override { if (node.body) node.body->accept(*this); }
    void visit(StructDecl &node) override {}
    void visit(EnumDecl &node) override {}
    void visit(TraitDecl &node) override {}
    void visit(ImplDecl &node) override {}
    void visit(Program &node) override {}
    void visit(MethodCallExpr &node) override { node.base->accept(*this); for (auto &arg : node.args) arg->accept(*this); }
    void visit(EnumVariantExpr &node) override { if (node.payload) node.payload->accept(*this); }
    void visit(ConstDecl &node) override { if (node.value) node.value->accept(*this); }
...
void Monomorphizer::specializeStruct(const std::string &name, const std::vector<TypeInfo> &args) {
    ...
    if (specializedStructs_.count(inst)) return;
    ...
    for (auto &sd : currentProgram_->structs) {
        if (sd->name == name) {
            ...
            auto specialized = std::unique_ptr<StructDecl>(static_cast<StructDecl*>(sd->clone().release()));
    ...
    if (!structTypeParams.empty()) {
        ...
        for (size_t i = 0; i < implCount; ++i) {
            ...
            if (!impl->typeParams.empty() && impl->targetType.structName == name) {
                ...
                auto specializedImpl = std::unique_ptr<ImplDecl>(static_cast<ImplDecl*>(impl->clone().release()));
...
void Monomorphizer::specializeFunction(const std::string &name, const std::vector<TypeInfo> &args) {
    Instantiation inst = {name, args};
    if (specializedFunctions_.count(inst)) return;
    ...
    for (size_t i = 0; i < currentProgram_->functions.size(); ++i) {
        ...
        if (fn->name == name) {
            ...
            auto specialized = std::unique_ptr<FunctionDecl>(static_cast<FunctionDecl*>(fn->clone().release()));
            specialized->name = mangled;
            specialized->typeParams.clear();
...
void Monomorphizer::specializeEnum(const std::string &name, const std::vector<TypeInfo> &args) {
    Instantiation inst = {name, args};
    if (specializedEnums_.count(inst)) return;
    ...
    for (size_t i = 0; i < currentProgram_->enums.size(); ++i) {
        ...
        if (ed->name == name) {
            ...
            auto specialized = std::unique_ptr<EnumDecl>(static_cast<EnumDecl*>(ed->clone().release()));
            specialized->name = mangled;
            specialized->typeParams.clear();
