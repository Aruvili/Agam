...
void SymbolTable::addBorrow(const std::string &name, bool isMutable) {
    if (!borrowHistory_.empty()) {
        ...
        if (info) {
            if (isMutable) info->mutablyBorrowed = true;
            else info->sharedBorrowCount++;
        }
    }
}
...
void SymbolTable::removeBorrow(const std::string &name, bool isMutable) {
    SymbolInfo* info = lookupMutable(name);
    if (info) {
        if (isMutable) info->mutablyBorrowed = false;
        else info->sharedBorrowCount = std::max(0, info->sharedBorrowCount - 1);
    }
...
const ImplInfo* SymbolTable::findImpl(const std::string &traitName, const TypeInfo &targetType) const {
...
const ImplInfo* SymbolTable::findImplForMethod(const TypeInfo &targetType, const std::string &methodName) const {
    auto typesMatch = [](const TypeInfo &pattern, const TypeInfo &target) -> bool {
        ...
        if (pattern.kind == target.kind || (pattern.isStruct && target.isStruct) || (pattern.isGeneric)) {
            ...
            if (pattern.isStruct && target.isStruct) {
                if (pattern.structName == target.structName) return true;
                // Match monomorphized names: Vector_int64 matches Vector
                if (target.structName.find(pattern.structName + "_") == 0) return true;
                return false;
            }
            
            if (pattern.isGeneric) return true;
    ...
    for (const auto &impl : impls_) {
        if (typesMatch(impl.targetType, targetType)) {
            for (const auto &m : impl.methods) {
                if (m.name == methodName) return &impl;
            }
        }
    }
    return nullptr;
...
void SymbolTable::declareStruct(const std::string &name, const std::vector<std::string> &typeParams, const std::vector<SymbolInfo> &fields) {
    StructInfo info;
    info.name = name;
...
void SymbolTable::declareEnum(const std::string &name, const std::vector<std::string> &typeParams, const EnumInfo &info) {
...
bool SymbolTable::isZoneInScope(const std::string &name) const {
    if (name.empty() || name == "global") return true;
