...
void DiagnosticRenderer::renderToTerminal(const DiagnosticEngine &engine, std::ostream &os) {
    ...
    for (const auto &diag : engine.diagnostics()) {
        ...
        switch (diag.level) {
            case DiagnosticLevel::Note:    levelStr = "குறிப்பு (note)";    levelColor = ANSI_CYAN; break;
            case DiagnosticLevel::Warning: levelStr = "எச்சரிக்கை (warning)"; levelColor = ANSI_YELLOW; break;
            case DiagnosticLevel::Error:   levelStr = "பிழை (error)";   levelColor = ANSI_RED; break;
            case DiagnosticLevel::Fatal:   levelStr = "முக்கிய பிழை (fatal)";   levelColor = ANSI_RED; break;
        }

        os << ANSI_BOLD << diag.loc.filename << ":" << diag.loc.line << ":" << diag.loc.column << ": " 
           << levelColor << levelStr << ": " << ANSI_RESET << ANSI_BOLD << diag.message << ANSI_RESET << "\n";
...
void DiagnosticRenderer::renderSnippet(const DiagnosticEngine &engine, const Diagnostic &diag, std::ostream &os) {
    if (diag.loc.filename.empty()) return;
    ...
    for (int i = 1; i < diag.loc.column; ++i) {
        if (i <= (int)line.size() && line[i-1] == '\t') os << "\t";
        else os << " ";
    }
...
void DiagnosticRenderer::renderToJSON(const DiagnosticEngine &engine, std::ostream &os) {
    ...
    for (size_t i = 0; i < engine.diagnostics().size(); ++i) {
        ...
        os << "  {\n";
        os << "    \"level\": \"" << (d.level == DiagnosticLevel::Error ? "error" : d.level == DiagnosticLevel::Warning ? "warning" : "note") << "\",\n";
